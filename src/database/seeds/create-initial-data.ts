...
async function bootstrap() {
  ...
  const savedCommunityPosts = await communityPostRepo.save([
    {
      ...
      authorId: user1.id,
      content:
        '대구 북구 쪽 코스트코 장보기 팁 공유해요. #코스트코 #장보기 #꿀팁',
      locale: 'ko',
    },
    {
      author: user2,
      authorId: user2.id,
      content:
        '서울 중구 근처 장보기 끝! 저녁 시간대 할인 품목 은근 많네요. #이마트 #할인',
      locale: 'ko',
    },
    {
      author: user3,
      authorId: user3.id,
      content:
        '연어/우유/계란 같이 사면 좋은 조합 추천받아요. #연어 #계란 #장바구니',
      locale: 'ko',
    },
    {
      author: user4,
      authorId: user4.id,
      content:
        '오늘은 1+1 상품만 골라서 장봤는데 생각보다 절약 많이 됐어요. #1+1 #절약',
      locale: 'ko',
    },
    {
      author: user1,
    ...
    },
    {
      ...
      authorId: user2.id,
      content:
        '비 오는 날엔 근처 마트만 돌려도 충분히 알뜰하게 장볼 수 있더라고요. #마트 #알뜰',
      locale: 'ko',
    },
    {
      author: user3,
      authorId: user3.id,
      content:
        '오늘 본 할인 중 제일 괜찮았던 건 냉동식품 코너였어요. #냉동식품 #할인',
      locale: 'ko',
    },
    {
      author: user4,
      authorId: user4.id,
      content:
        '커뮤니티가 조금 더 북적이면 좋겠어서 첫 글 남깁니다. 다들 장보기 정보 많이 공유해요! #커뮤니티 #첫글',
      locale: 'ko',
    },
    {
      author: user1,
      authorId: user1.id,
      content:
        '北区のコストコで買い物する時のコツを共有します。 #コストコ #買い物 #節約',
      locale: 'ja',
    },
    {
      author: user2,
  ...
  await communityLikeRepo.save([
    ...
    {
      post: savedCommunityPosts[1],
      postId: savedCommunityPosts[1].id,
      user: user1,
      userId: user1.id,
    },
    {
    ...
    {
      post: savedCommunityPosts[6],
      postId: savedCommunityPosts[6].id,
      user: user1,
      userId: user1.id,
    },
    {
    ...
    {
      post: savedCommunityPosts[10],
      postId: savedCommunityPosts[10].id,
      user: user4,
      userId: user4.id,
    },
  ]);
  ...
  await chatRepo.save([
    {
      party: savedParties[0],
      sender: user1,
      content: '안녕하세요! 고기 나누실 분?',
    },
    {
      party: savedParties[0],
      sender: user2,
      content: '저요! 저도 마침 사려던 참이었어요.',
    },
  ]);
